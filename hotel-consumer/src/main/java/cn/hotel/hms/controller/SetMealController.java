...
public class SetMealController {
    ...
    @RequestMapping(value = "toUpdateSetMeal")
    public String toUpdateSetMeal(Integer id, Model model){
        model.addAttribute("setmeal",this.setMealService.detaSetMeal(id));
        return "food/setmeal_updates";
    }
    @RequestMapping(value = "doUpdateSetMeal")
    public String doUpdateSetMeal(MultipartFile file, SetMeal setMeal, BindingResult result){
        String fillName=fileUtil.upload(file,1);
        setMeal.setMealOne(logPath+fillName);
        this.setMealService.updateSetMeal(setMeal);
        return "redirect:queryAllMeal";
    }
